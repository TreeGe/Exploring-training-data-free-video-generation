...
class EditingPipeline(BasePipeline):
    def __call__(
        ...
        x_in = x_in.to(dtype=self.unet.dtype, device=self._execution_device)  #1，4，64，64
        ...
        with torch.no_grad():
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps): #采样50步
        ...
        images=[]
        np1 = list(arange(1,math.e,(math.e-1)/samples_times,'d')+(math.e-1)/samples_times)
        for i in range(1,samples_times+1):
            edit_temp = edit_dir.clone()
            edit_temp = edit_temp*math.log(np1[i-1])
            print(edit_temp)
            prompt_embeds_edit = prompt_embeds.clone() #2×77×768  原图像文本提示的编码
            prompt_embeds_edit[1:2] += edit_temp
            #编辑方向加入prompt_embeds_edit

            latents = latents_init  #源图像的加入步长为50的噪声
